...
export default function JoinHackathonPage() {
  ...
  const [joinHackathon, { loading: joining }] = useMutation(JOIN_HACKATHON, {
    ...
    onError: (error) => {
      ...
      toast(`Failed to join hackathon: ${error.message}`);
    },
  });

  const handleJoinHackathon = async (hackathonId: string) => {
    if (!username) {
      toast("Please log in to join a hackathon");
      router.push("/login");
      return;
    }
    ...
    try {
      ...
      toast("Successfully joined the hackathon!");
    } catch (error) {
  ...
  const filteredHackathons =
    data?.getHackathons?.filter((hackathon: any) => {
        ...
        {filteredHackathons.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredHackathons.map((hackathon: any) => {
              const startDate = hackathon.date.replace("T", " ").slice(0, 10);
              const endDate = hackathon.endDate.replace("T", " ").slice(0, 10);
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
                      <div className="flex items-center">
                        <Users className="h-4 w-4 mr-1" /> {participantCount} /{" "}
